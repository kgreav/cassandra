...
// TODO: make this work for migration to new batchlog design from 3.0 design.
public final class LegacyBatchlogMigrator
{
    ...
    {
        ColumnFamilyStore store = Keyspace.open(SchemaConstants.SYSTEM_KEYSPACE_NAME).getColumnFamilyStore(SystemKeyspace.LEGACY_BATCHLOG_V2);
        ...
        String query = String.format("SELECT id, mutation, version FROM %s.%s",
        ...
        if (convertedBatches > 0)
            Keyspace.openAndGetStore(SystemKeyspace.LegacyBatchlogV2).truncateBlocking();
    }
    ...
    public static boolean isLegacyBatchlogMutation(Mutation mutation)
    {
        return mutation.getKeyspaceName().equals(SchemaConstants.SYSTEM_KEYSPACE_NAME)
            && mutation.getPartitionUpdate(SystemKeyspace.LegacyBatchlogV2) != null;
    }

    @SuppressWarnings("deprecation")
    public static void handleLegacyMutation(Mutation mutation)
    {
        PartitionUpdate update = mutation.getPartitionUpdate(SystemKeyspace.LegacyBatchlogV2);
    ...
    private static boolean apply(UntypedResultSet.Row row, long counter)
    {
        ...
        long timestamp = UUIDGen.unixTimestamp(id);
        int version = row.has("version") ? row.getInt("version") : MessagingService.VERSION_30;

        logger.trace("Converting mutation at {}", timestamp);

        for (ByteBuffer mut : row.getList("mutations", BytesType.instance))
        {
            try (DataInputBuffer in = new DataInputBuffer(row.getBytes("mutation"), false))
            {
    ...
    {
//        return new RowUpdateBuilder(SystemKeyspace.LegacyBatchlogV2, batch.creationTime, batch.id)
//               .clustering()
...
//               .add("version", version);
    ...
    {
        return new Mutation(PartitionUpdate.fullPartitionDelete(SystemKeyspace.LegacyBatchlogV2,
